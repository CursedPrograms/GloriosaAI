...
if not os.path.exists(generator_weights_path) or not os.path.exists(discriminator_weights_path):
    ...
    # Load the current epoch from the "current_epoch.txt" file
    epoch_file_path = os.path.join(checkpoint_dir, "current_epoch.txt")
    if os.path.exists(epoch_file_path):
        with open(epoch_file_path, "r") as epoch_file:
            initial_epoch = int(epoch_file.read())
        print(f"Loaded initial epoch from file: {initial_epoch}")
    else:
        print("Epoch file not found. Starting from epoch 0.")
...
def save_and_trigger_callbacks(generator, discriminator, epoch, checkpoint_dir):
    ...
    epoch_file_path = os.path.join(checkpoint_dir, "current_epoch.txt")
    with open(epoch_file_path, "w") as epoch_file:
