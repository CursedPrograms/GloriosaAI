...
image_shape = (28, 28, 3)  # RGB images with size 28x28

os.makedirs(output_dir, exist_ok=True)
os.makedirs(model_save_dir, exist_ok=True)

# Define a function to load and preprocess your dataset
def load_and_preprocess_dataset(target_size):
    ...
    dataset = datagen.flow_from_directory(
        dataset_dir,  # Use the training_data folder relative to the script
        target_size=target_size,
        batch_size=batch_size,
        class_mode='input'  # Use 'input' mode for autoencoders
    )
    return dataset

# Load and preprocess the dataset
dataset = load_and_preprocess_dataset(image_shape[:2])  # Adjust target_size
...
def generate_and_save_images(generator, epoch, output_dir, num_examples=16):
    ...
    plt.savefig(os.path.join(output_dir, f"generated_image_epoch_{epoch}.png"))  # Save the figure
...
for epoch in range(epochs):
    ...
    for _ in range(0, len(dataset), batch_size):
        ...
        real_images = real_batch[0]  # Extract real images from the batch
        real_labels = np.ones((real_images.shape[0], 1))  # Match the batch size

        noise = np.random.normal(0, 1, (real_images.shape[0], input_dim))

        generated_images = generator.predict(noise)
        fake_labels = np.zeros((real_images.shape[0], 1))  # Match the batch size
